...
public enum Query {
    ...
    ISLAND_SET_BANK_LIMIT("UPDATE {prefix}islands SET bankLimit=? WHERE owner=?;"),
    ISLAND_SET_LAST_INTEREST("UPDATE {prefix}islands SET lastInterest=? WHERE owner=?;"),
    ISLAND_SET_ROLE_LIMITS("UPDATE {prefix}islands SET roleLimits=? WHERE owner=?;"),
    ISLAND_SET_WARP_CATEGORIES("UPDATE {prefix}islands SET warpCategories=? WHERE owner=?;"),
    ISLAND_UPDATE("UPDATE {prefix}islands SET teleportLocation=?,visitorsLocation=?,members=?,banned=?,permissionNodes=?,upgrades=?,warps=?,islandBank=?,islandSize=?,blockLimits=?,teamLimit=?,cropGrowth=?,spawnerRates=?,mobDrops=?,discord=?,paypal=?,warpsLimit=?,bonusWorth=?,locked=?,blockCounts=?,name=?,description=?,ratings=?,settings=?,ignored=?,generator=?,generatedSchematics=?,schemName=?,uniqueVisitors=?,unlockedWorlds=?,lastTimeUpdate=?,dirtyChunks=?,entityLimits=?,bonusLevel=?,creationTime=?,coopLimit=?,islandEffects=?,islandChest=?,uuid=?,bankLimit=?,lastInterest=?,roleLimits=?,warpCategories=? WHERE owner=?;"),
    ISLAND_INSERT("REPLACE INTO {prefix}islands (owner,center,teleportLocation,members,banned,permissionNodes,upgrades,warps,islandBank,islandSize,blockLimits,teamLimit,cropGrowth,spawnerRates,mobDrops,discord,paypal,warpsLimit,bonusWorth,locked,blockCounts,name,visitorsLocation,description,ratings,settings,ignored,generator,generatedSchematics,schemName,uniqueVisitors,unlockedWorlds,lastTimeUpdate,dirtyChunks,entityLimits,bonusLevel,creationTime,coopLimit,islandEffects,islandChest,uuid,bankLimit,lastInterest,roleLimits,warpCategories) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"),
    ...
    PLAYER_UPDATE("UPDATE {prefix}players SET teamLeader=?,name=?,islandRole=?,textureValue=?,disbands=?,toggledPanel=?,islandFly=?,borderColor=?,lastTimeStatus=?,language=?,toggledBorder=? WHERE player=?;"),
    PLAYER_INSERT("REPLACE INTO {prefix}players (player,teamLeader,name,islandRole,textureValue,disbands,toggledPanel,islandFly,borderColor,lastTimeStatus,language,toggledBorder) VALUES(?,?,?,?,?,?,?,?,?,?,?,?);"),
