...
public class SignsListener extends AbstractGameEventListener {
    ...
    private void onSignChange(GameEvent<GameEventArgs.SignChangeEvent> e) {
        ...
        // Visitor homes and warps cannot be set using hanging signs.
        ...
        try (ObjectsPools.Wrapper<Location> wrapper = ObjectsPools.LOCATION.obtain()) {
            ...
            newLines = e.getArgs().lines.clone();

            result = IslandSigns.handleSignPlace(superiorPlayer, location, newLines);
        }
        ...
        // We update the sign only if its lines have been modified.
        if (Arrays.equals(newLines, e.getArgs().lines)) {
            return;
        }
    ...
    private void registerListeners() {
        registerCallback(GameEventType.SIGN_CHANGE_EVENT, GameEventPriority.MONITOR, this::onSignChange);
    ...
    private static boolean isValidIsland(Island island) {
        return island != null && (!island.getIslandWarps().isEmpty() || !island.getVisitorHomesPositions().isEmpty());
    }
