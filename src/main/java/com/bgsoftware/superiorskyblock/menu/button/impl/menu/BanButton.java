...
public final class BanButton extends SuperiorMenuButton {
    ...
    public void onButtonClick(SuperiorSkyblockPlugin plugin, SuperiorMenu superiorMenu, InventoryClickEvent clickEvent) {
        Preconditions.checkArgument(superiorMenu instanceof MenuConfirmBan, "superiorMenu must be MenuConfirmBan");

        MenuConfirmBan menuConfirmBan = (MenuConfirmBan) superiorMenu;

        SuperiorPlayer clickedPlayer = plugin.getPlayers().getSuperiorPlayer(clickEvent.getWhoClicked());

        if (banPlayer)
            IslandUtils.handleBanPlayer(clickedPlayer, menuConfirmBan.getTargetIsland(), superiorMenu.getTargetPlayer());

        Executor.sync(superiorMenu::closePage, 1L);
    }

    public static class Builder extends AbstractBuilder<Builder, BanButton> {
