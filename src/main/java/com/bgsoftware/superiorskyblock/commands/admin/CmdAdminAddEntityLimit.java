...
public final class CmdAdminAddEntityLimit implements ISuperiorCommand {
    ...
    public void execute(SuperiorSkyblockPlugin plugin, CommandSender sender, String[] args) {
        ...
        Executor.data(() -> islands.forEach(island -> island.setEntityLimit(entityType, island.getEntityLimit(entityType) + limit)));
        ...
        if(args.length == 3){
            ...
            if(island != null){
                for(EntityType entityType : EntityType.values()){
                    if(EntityUtils.canHaveLimit(entityType) && entityType.name().toLowerCase().startsWith(args[3].toLowerCase()))
