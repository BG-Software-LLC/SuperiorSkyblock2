...
public class CmdAdminSetBlockLimit implements InternalIslandsCommand {
    ...
    public void execute(SuperiorSkyblockPlugin plugin, IslandsCommandContext context) {
        ...
        for (Island island : islands) {
            EventResult<Integer> eventResult = plugin.getEventsBus().callIslandChangeBlockLimitEvent(dispatcher, island, key, limit);
            anyIslandChanged |= !eventResult.isCancelled();
            if (!eventResult.isCancelled()) island.setBlockLimit(key, eventResult.getResult());
        }
