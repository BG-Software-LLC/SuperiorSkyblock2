...
public final class CmdAdminAddGenerator implements IAdminIslandCommand {
    ...
    public String getDescription(java.util.Locale locale) {
        return Locale.COMMAND_DESCRIPTION_ADMIN_ADD_GENERATOR.getMessage(locale);
    }
    ...
    public void execute(SuperiorSkyblockPlugin plugin, CommandSender sender, SuperiorPlayer targetPlayer, List<Island> islands, String[] args) {
        ...
        Executor.data(() -> islands.forEach(island -> {
            if(percentage){
                island.setGeneratorPercentage(material, Math.max(0, Math.min(100, island.getGeneratorPercentage(material) + amount)));
            }
            else{
                island.setGeneratorAmount(material, island.getGeneratorAmount(material) + amount);
            }
        }));
