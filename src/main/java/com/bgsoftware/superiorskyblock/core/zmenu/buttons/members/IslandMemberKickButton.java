...
public class IslandMemberKickButton extends SuperiorButton {
    public IslandMemberKickButton(Plugin plugin) {
    ...
    public void onClick(Player player, InventoryClickEvent event, InventoryDefault inventory, int slot, Placeholders placeholders) {
        ...
        if (plugin.getSettings().isKickConfirm()) {
            ...
            if (IslandUtils.checkKickRestrictions(inventoryViewer, island, targetPlayer)) {
                plugin.getMenus().openConfirmKick(inventoryViewer, null, island, targetPlayer);
            }
        } else {
            plugin.getCommands().dispatchSubCommand(player, "kick", targetPlayer.getName());
        }
    }
}
