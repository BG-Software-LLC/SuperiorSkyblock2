    ...
    public void onRender(Player player, InventoryDefault inventory) {
        ...
        for (int i = 0; i != Math.min(blockCounts.size(), this.slots.size()); i++) {
            ...
            ItemStack itemStack = itemBuilder
                    .withName(getItemStack().getDisplayName())
                    .withLore(getItemStack().getLore())
                    .withAmount(BigInteger.ONE.max(MAX_STACK.min(amount.toBigInteger())).intValue())
                    .replaceAll("%material-name%", Formatters.CAPITALIZED_FORMATTER.format(materialName))
                    .replaceAll("%quantity%", String.valueOf(amount))
                    .replaceAll("%worth%", Formatters.NUMBER_FORMATTER.format(worthValue.multiply(amount)))
                    .replaceAll("%level%", Formatters.NUMBER_FORMATTER.format(levelValue.multiply(amount)))
                    .replaceAll("%worth-formatted%", Formatters.FANCY_NUMBER_FORMATTER.format(worthValue.multiply(amount), inventoryViewer.getUserLocale()))
                    .replaceAll("%level-formatted%", Formatters.FANCY_NUMBER_FORMATTER.format(levelValue.multiply(amount), inventoryViewer.getUserLocale()))
