...
import com.bgsoftware.superiorskyblock.nms.v1_18_R2.spawners.MobSpawnerAbstractNotifier;
        ...
        try {
            ...
            if (disabled || island == null || (!plugin.getSettings().getSpawn().isWorldBorder() && island.isSpawn())) {
                ...
                switch (superiorPlayer.getBorderColor()) {
                    case GREEN ->
                            worldBorder.transitionSizeBetween(worldBorderSize - 0.1D, worldBorderSize, Long.MAX_VALUE);
                    case RED ->
                            worldBorder.transitionSizeBetween(worldBorderSize, worldBorderSize - 1.0D, Long.MAX_VALUE);
                }
            }
