...
public final class NMSAdapter_v1_8_R1 implements NMSAdapter {
    ...
    public void setWorldBorder(SuperiorPlayer superiorPlayer, Island island) {
        try {
            ...
            if(!superiorPlayer.hasWorldBorderEnabled() || island == null || (!plugin.getSettings().spawnWorldBorder && island.isSpawn())){
                worldBorder = ((CraftWorld) superiorPlayer.getWorld()).getHandle().af();
            }
            else {
                worldBorder = new WorldBorder();

                worldBorder.world = ((CraftWorld) superiorPlayer.getWorld()).getHandle();
                double size = island.getIslandSize() + 1;
    ...
    public void sendTitle(Player player, String title, String subtitle, int fadeIn, int duration, int fadeOut) {
        ...
        if (title != null) {
            times = new PacketPlayOutTitle(EnumTitleAction.TITLE, CraftChatMessage.fromString(title)[0]);
            playerConnection.sendPacket(times);
        }

        if (subtitle != null) {
            times = new PacketPlayOutTitle(EnumTitleAction.SUBTITLE, CraftChatMessage.fromString(subtitle)[0]);
            playerConnection.sendPacket(times);
        }
